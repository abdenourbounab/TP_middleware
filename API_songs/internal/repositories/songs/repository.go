...
func GetAllSongs() ([]models.Songs, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&data.id, &data.Album, &data.Artist, &data.Artist, &data.Song_name, &data.Duration, &data.Type, &data.Playlist)
		if err != nil {
			return nil, err
		}
...
func GetSongById(id uuid.UUID) (*models.Songs, error) {
	...
	err = row.Scan(&song.id, &song.Artist, &song.Song_name)
	if err != nil {
		return nil, err
	}
	return &song, err
